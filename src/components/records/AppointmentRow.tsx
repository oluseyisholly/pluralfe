...
export function AppointmentRow({ appointment }: AppointmentRowProps) {
  return (
    ...
    <div className="flex items-center gap-3">
      <Avatar fallback={getInitials(appointment.patient.name)} className="h-12 w-12" />
      <div>
        ...
        <p className="text-xs text-slate-500">
          {appointment.patient.id} • {appointment.patient.sex} • {appointment.patient.age} yrs
        </p>
      </div>
    </div>
    ...
    <div className="flex min-w-[110px] flex-col text-right text-sm font-semibold text-indigo-600 sm:items-end">
      ...
      <span className="text-xs font-medium text-slate-400">
        <CalendarClock className="mr-1 inline h-3.5 w-3.5 text-slate-300" aria-hidden="true" />
    ...
    <div className="flex items-center justify-end">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger aria-label={`Actions for ${appointment.patient.name}`}>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem className="text-rose-600 hover:text-rose-700">Cancel appointment</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
