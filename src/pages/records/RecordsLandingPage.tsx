...
import type { AppointmentsQueryParams } from "../../services/appointments";
...
export function RecordsLandingPage() {
  ...
  const queryInput = useMemo(
    () => {
      const payload: AppointmentsQueryParams = {
        page,
        pageSize: PAGE_SIZE,
        sort,
      };
  ...
  const {
    ...
    error,
    refetch,
  } = useAppointmentsQuery(queryInput);

  const appointments = data?.items ?? [];
  const applyLocalProcessing = data?.fromFallback ?? true;
    ...
    <AppLayout currentDate={new Date()} userName="Mr Daniel">
      <section className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-4 px-4 pb-6 md:px-10">
          {!isLoading && data?.fromFallback ? (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-600">
              Displaying cached appointments while the live service is unavailable.
            </div>
          ) : null}
          <AppointmentsList
            ...
            onRetry={refetch}
            applyLocalProcessing={applyLocalProcessing}
